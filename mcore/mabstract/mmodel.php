...
abstract class MmodelCore { 
    ...
    /**
     * @var array Label, note and placeholder array
     */
    ...
    public function validate(){
        ...
        foreach ( $this->attributes as $attrib => $value ){
            ...
            if( isset( $rules['required'] ) && preg_match( '/\b'.$attrib.'\b/', $rules['required']) != 0 ){
                ...
                if( $this->primaryKey !== $attrib && !in_array( $attrib, $this->columnsWithDefValue ) 
                        && ( !isset( $value ) || empty( $value ) ) ){
                    
                    $this->validationErrors .= 
                            '<li><strong>' . (isset( $labels[$attrib] )? $labels[$attrib] : $attrib) 
                            . '</strong> je povinné pole</li>';
                }
            }
            // Dlzka retazca
            ...
            if( isset( $rules['unique'] ) && strpos( $rules['unique'], $attrib ) !== false ){
                ...
                if( $this->isNewRecord ){
                    $query = "SELECT GROUP_CONCAT( '--S_E_P--', t.{$attrib}, '--S_E_P--' SEPARATOR '') as toCheck from {$this->table()} t;";
                }else{
                    $query = "SELECT GROUP_CONCAT( '--S_E_P--', t.{$attrib}, '--S_E_P--' SEPARATOR '--SEP--') as toCheck from {$this->table()} t WHERE t.id != {$this->id};";
                }
                Mcore::base()->getObject('db')->executeQuery( $query );
                $result = Mcore::base()->getObject('db')->getArrayRows();
                
                if( isset( $result[0]['toCheck'] ) && isset( $value ) 
                        && @preg_match( '/--S_E_P--'.$value.'--S_E_P--/', $result[0]['toCheck']) != 0 ){
                     $this->validationErrors .= 
                            '<li>Hodnota v poli <strong>' . ( isset( $labels[$attrib] )? $labels[$attrib] : $attrib) 
                            . '</strong> sa už používa.</li>';
                }
            }
            if( isset( $rules['format'] ) ){
                if( isset( $rules['format']['email'] ) 
                        && preg_match( '/\b'.$attrib.'\b/', $rules['format']['email']) != 0 && !empty( $value ) ) {
    ...
    public function delete ( ) {
        ...
        return Mcore::base()->getObject('db')->deleteRecords( $this->tableName, 
                    "{$this->primaryKey} = '{$this->attributes[$this->primaryKey]}'" );
    }
    
    /**
     * Metóda, ktorá by mala vrátiť pole definovaných označení pre jednotlivé atribúty modelu
     * @return array
     */
    ...
    /**
     * Abstraktná metóda, ktorá by mala vracať názov tabuľky v DB, ku ktorej prislúcha model
     */
    abstract public function table();
    
    /** 
     * Abstraktná metóda, ktorá by mala vrátiť pole definovaných pravidiel pre jednotlivé atribúty modelu
     */
